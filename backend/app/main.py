...
# Create tables automatically (works with SQLite for local dev;
# use `alembic upgrade head` for PostgreSQL in production).
...
def analytics_estimate_price(payload: PriceEstimateInput) -> PriceEstimateOutput:
    ...
    return PriceEstimateOutput(
        ...
        confidence_note="Baseline MVP estimate. Improve with real historical data.",
    )
